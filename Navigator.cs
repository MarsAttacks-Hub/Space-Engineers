...
namespace IngameScript {
    partial class Program : MyGridProgram {
        ...
        Vector3D CalculateThrustStopVector(double timeStep, Vector3D myVelocity) {//, float myMass = CONTROLLER.CalculateShipMass().PhysicalMass
            ...
            for (int i = 0; i < 3; ++i) {//Now we store off the components if they are larger (in magnitude) than what we have stored
                ...
                if (component >= 0d) {
                    if (component > thrustVector.GetDim(i)) {//Bigger than what we have stored
                        stopVector.SetDim(i, component);
                    }
                } else {//if negative
                    component = Math.Abs(component);
                    if (component > thrustVector.GetDim(i)) {//Bigger (in magnitude) than what we have stored
                        stopVector.SetDim(i, component);
                    }
                }
            }
            stopVector = Vector3D.TransformNormal(stopVector, MatrixD.Transpose(CONTROLLER.WorldMatrix));
            return stopVector;
        }
        ...
        void Descend(Vector3D myVelocity, Vector3D gravity) {
            ...
            Vector3D localGravity = Vector3D.TransformNormal(gravity, MatrixD.Transpose(CONTROLLER.WorldMatrix));
            myThrustList.GetThrustVectorByDirection(localGravity);
            weight = (weight / 10d) * 9d;//90% thrust
